...
class RiscvAttributeDirective(RiscvDirective):
    directives = ".attribute"

    def __init__(self, mnemonic: str, args: list[str], supported_extensions: set[str]):
        super().__init__(mnemonic, args)
        self._check_extensions(supported_extensions)

    def _check_extensions(self, supported_extensions):
        if self.get_args()[0] == "arch":
            extensions = set(
                map(lambda s: s[:-3], self.get_args()[1].strip('"')[4:].split("_"))
            )
            print(*extensions)
            if not (extensions <= supported_extensions):
                raise Exception("Check for extension support")


class RiscvSectionDirective(RiscvDirective):
    section_start = [".section", ".bss", ".rodata", ".data", ".text", ".sbss", ".sdata", ".srodata"]
